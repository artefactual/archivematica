...
def test_worker_metric_queue_failures_are_logged_once(
    ...
    monkeypatch.setattr(metrics.settings, "PROMETHEUS_ENABLED", True)
    monkeypatch.setattr(metrics, "_QUEUE_WRITE_FAILURE_LOGGED", False)
    caplog.set_level(logging.WARNING, logger=metrics.logger.name)

    metrics.configure_event_queue(FailingQueue())
    try:
        metrics.job_completed("copy_v0.0")
    finally:
        metrics.configure_event_queue(None)

    assert caplog.text.count("Dropping MCPClient metric event") == 1
...
def test_registry_preserves_mcpclient_metric_contract(monkeypatch) -> None:
    ...
    expected_types = {
        ...
        "mcpclient_metric_events_applied_total": "counter",
        "mcpclient_metric_event_apply_errors_total": "counter",
    }
    ...
    for line in output.splitlines():
        if line.startswith("# TYPE "):
            ...
            actual_types[metric_name] = metric_type

    assert {
        metric_name: actual_types.get(metric_name) for metric_name in expected_types
    } == expected_types
