...
def test_metrics_import_resets_prometheus_multiprocess_mode(
    ...
    pythonpath = [
        str(repo_root / "src" / "MCPClient" / "lib"),
        str(repo_root / "src" / "archivematicaCommon" / "lib"),
        str(repo_root / "src" / "dashboard" / "src"),
    ]
...
def test_registry_preserves_mcpclient_metric_contract(monkeypatch, tmp_path) -> None:
    modules_file = tmp_path / "clientModules.conf"
    modules_file.write_text(
        "[supportedBatchCommands]\n" "copy_v0.0 = copy.py\n",
        encoding="utf8",
    )
    monkeypatch.setattr(metrics.settings, "PROMETHEUS_DETAILED_METRICS", False)
    monkeypatch.setattr(metrics.settings, "CLIENT_MODULES_FILE", str(modules_file))
