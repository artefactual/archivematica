...
import (
	...
	"dagger/ccp/internal/querybuilder"
)
...
// The `GolangciLintID` scalar type represents an identifier for an object of type GolangciLint.
...
type WithGolangciLintFunc func(r *GolangciLint) *GolangciLint

// With calls the provided function with current GolangciLint.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *GolangciLint) With(f WithGolangciLintFunc) *GolangciLint {
	return f(r)
}
...
func (r *GolangciLint) Container() *Container {
...
func (r *GolangciLint) ID(ctx context.Context) (GolangciLintID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.query.Select("id")

	var response GolangciLintID
...
func (r *GolangciLint) UnmarshalJSON(bs []byte) error {
	...
	*r = *dag.LoadGolangciLintFromID(GolangciLintID(id))
	return nil
}

// GolangciLintRunOpts contains options for GolangciLint.Run
type GolangciLintRunOpts struct {
	//
...
func (r *GolangciLint) Run(source *Directory, opts ...GolangciLintRunOpts) *Container {
	assertNotNil("source", source)
...
// GolangciLintWithBuildCacheOpts contains options for GolangciLint.WithBuildCache
type GolangciLintWithBuildCacheOpts struct {
	//
	// Identifier of the directory to use as the cache volume's root.
	//
	Source *Directory
...
func (r *GolangciLint) WithBuildCache(cache *CacheVolume, opts ...GolangciLintWithBuildCacheOpts) *GolangciLint {
	assertNotNil("cache", cache)
...
// GolangciLintWithLinterCacheOpts contains options for GolangciLint.WithLinterCache
type GolangciLintWithLinterCacheOpts struct {
	//
	// Identifier of the directory to use as the cache volume's root.
	//
	Source *Directory
...
func (r *GolangciLint) WithLinterCache(cache *CacheVolume, opts ...GolangciLintWithLinterCacheOpts) *GolangciLint {
	assertNotNil("cache", cache)
	q := r.query.Select("withLinterCache")
	for i := len(opts) - 1; i >= 0; i-- {
		// `source` optional argument
		if !querybuilder.IsZeroValue(opts[i].Source) {
			q = q.Arg("source", opts[i].Source)
...
// GolangciLintWithModuleCacheOpts contains options for GolangciLint.WithModuleCache
type GolangciLintWithModuleCacheOpts struct {
	//
	// Identifier of the directory to use as the cache volume's root.
	//
	Source *Directory
...
func (r *GolangciLint) WithModuleCache(cache *CacheVolume, opts ...GolangciLintWithModuleCacheOpts) *GolangciLint {
	assertNotNil("cache", cache)
...
// GolangciLintOpts contains options for Client.GolangciLint
...
func (r *Client) GolangciLint(opts ...GolangciLintOpts) *GolangciLint {
	...
	for i := len(opts) - 1; i >= 0; i-- {
		...
		// `linterCache` optional argument
		if !querybuilder.IsZeroValue(opts[i].LinterCache) {
			q = q.Arg("linterCache", opts[i].LinterCache)
		}
		// `goVersion` optional argument
		if !querybuilder.IsZeroValue(opts[i].GoVersion) {
			q = q.Arg("goVersion", opts[i].GoVersion)
		}
		// `goImage` optional argument
		if !querybuilder.IsZeroValue(opts[i].GoImage) {
			q = q.Arg("goImage", opts[i].GoImage)
		}
		// `goContainer` optional argument
		if !querybuilder.IsZeroValue(opts[i].GoContainer) {
			q = q.Arg("goContainer", opts[i].GoContainer)
		}
		// `disableGoCache` optional argument
		if !querybuilder.IsZeroValue(opts[i].DisableGoCache) {
			q = q.Arg("disableGoCache", opts[i].DisableGoCache)
		}
	}
...
// Load a GolangciLint from its ID.
func (r *Client) LoadGolangciLintFromID(id GolangciLintID) *GolangciLint {
	q := r.query.Select("loadGolangciLintFromID")
	q = q.Arg("id", id)
