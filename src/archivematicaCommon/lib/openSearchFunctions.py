...
class EmptySearchResultError(OpenSearchError):
    pass


class TooManyResultsError(OpenSearchError):
    pass
...
def setup(hosts, timeout=DEFAULT_TIMEOUT, enabled=(AIPS_INDEX, TRANSFERS_INDEX)):
    """Initialize and share the OpenSearch client.

    Share it as the attribute _os_client in the current module. An additional
    attribute _os_hosts is defined containing the OpenSearch hosts (expected
    ...
    _os_client = OpenSearch(
        **{"hosts": _os_hosts, "timeout": timeout, "dead_timeout": 2}
    )
    ...
    if len(indexes) > 0:
        create_indexes_if_needed(_os_client, indexes)
    else:
        logger.warning("Setting up the OpenSearch client " "without enabled indexes.")


def setup_reading_from_conf(settings):
    setup(
        settings.OPENSEARCH_SERVER,
        settings.OPENSEARCH_TIMEOUT,
        settings.SEARCH_ENABLED,
    )


def get_client():
    """Obtain the current OpenSearch client.
    ...
    if not _os_client:
        raise ImproperlyConfigured(
            "The OpenSearch client has not been set up yet. Please call setup() first."
        )
    return _os_client
...
def _try_to_index(
    ...
    # reraise the OpenSearch exception to aid in debugging.
    if exception:
        raise exception
...
def _normalize_dict(data):
    ...
    problems in the OpenSearch index as it expects consistent types. This
...
def get_file_tags(client, uuid):
    ...
    :param OpenSearch client: OpenSearch client
...
def get_transfer_file_info(client, field, value):
    """
    Get transferfile information from OpenSearch with query field = value.
    """
    ...
    if result_count == 1:
        ...
        # OpenSearch was sometimes ranking results for a different filename above
        ...
        if result_count > 1:
            ...
            raise OpenSearchError("get_transfer_file_info returned no exact results")
