...
def get_purl(pid: Any, naming_authority: Any, resolver_url: Any) -> str:
...
def get_qualified_purl(purl: Any, qualifier: Any) -> str:
...
def _render_template(template: str, _params: Any) -> Any:
...
def _render_url_templates(
    argsdict: Dict[Any, Any]
) -> Tuple[Any, List[Dict[str, Any]], Dict[Any, Any]]:
...
def _render_request_body(
    argsdict: Any, resolve_url: Any, qualified_resolve_urls: List[Dict[str, Any]]
) -> Any:
...
def _get_config_val(config: configparser.RawConfigParser, key: str) -> Optional[Any]:
...
def get_command_line_params() -> Dict[str, Any]:
