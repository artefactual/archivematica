...
class RightsForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['rightsbasis'].widget.attrs['title'] = _("Designation of the basis for the right or permission described in the rights statement identifier.")
...
class RightsGrantedForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'act': forms.widgets.TextInput(attrs={'class': 'span11', 'title': _("The action the preservation repository is allowed to take; eg replicate, migrate, modify, use, disseminate, delete")}),
            'restriction': forms.widgets.TextInput(attrs=settings.INPUT_ATTRS),
            'startdate': forms.widgets.TextInput(attrs={'class': 'span11', 'title': _("The beginning date of the rights or restrictions granted")}),
...
class RightsCopyrightForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'copyrightstatus': forms.widgets.Select(attrs={'class': 'span11', 'title': _("A coded designation of the copyright status of the object at the time the rights statement is recorded. Available options: Copyrighted, Public Domain, Unknown")}),
            'copyrightjurisdiction': forms.widgets.TextInput(attrs={'class': 'span11', 'title': _("The country whose copyright laws apply [ISO 3166]")}),
            'copyrightstatusdeterminationdate': forms.widgets.TextInput(attrs={'class': 'span11', 'title': _("The date that the copyright status recorded in 'copyright status' was determined.")}),
...
class RightsStatuteForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'statutejurisdiction': forms.widgets.TextInput(attrs={'class': 'span11', 'title': _("The country or other political body enacting the statute.")}),
            'statutecitation': forms.widgets.TextInput(attrs={'class': 'span11', 'title': _("An identifying designation for the statute.")}),
            'statutedeterminationdate': forms.widgets.TextInput(attrs={'class': 'span11', 'title': _("The date that the determination was made that the statue authorized the permission(s) noted.")}),
...
class RightsOtherRightsForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'otherrightsbasis': forms.widgets.TextInput(attrs={'class': 'span11', 'title': _("The designation of the basis for the other right or permission described in the rights statement identifier.")}),
