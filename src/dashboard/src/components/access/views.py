...
def _authenticate_to_archivesspace(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            response = {
                "success": False,
                "message": "Unable to connect to Archivist's Toolkit database; failed with error: {}".format(
                    e
                ),
            }
            return django.http.HttpResponseServerError(
    ...
    def wrapper(request, system="", record_id=""):
        try:
            mapping = SIPArrangeAccessMapping.objects.get(
                system=system, identifier=_normalize_record_id(record_id)
            )
...
def record_children(client, request, system="", record_id=""):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            response = {
                "success": False,
                "message": "MySQL error encountered while communicating with Archivist's Toolkit: "
                + str(e),
            }
            return helpers.json_response(response, status_code=400)
...
def get_digital_object_component_path(
    record_id, component_id, system="archivesspace", create=True
):
    mapping = create_arranged_directory(system, record_id)
    ...
    if request.method == "POST":
        ...
        try:
            access_path = get_digital_object_component_path(
                record_id, component.id, system=system, create=True
            )
        ...
        for component in components:
            access_path = get_digital_object_component_path(
                record_id, component["id"], system=system
            )
...
@_authenticate_to_archivesspace
def digital_object_components_files(
    client, request, system="archivesspace", record_id="", component_id=""
):
    access_path = get_digital_object_component_path(
        record_id, component_id, system=system
    )
...
def _get_sip_arrange_children(record, system):
    ...
    try:
        mapping = SIPArrangeAccessMapping.objects.get(
            system=system, identifier=record["id"]
        )
