...
urlpatterns = [
    ...
    url(
        r"(?P<system>archivesspace|atk)/(?P<record_id>[A-Za-z0-9-_]+)/contents/arrange/$",
        views.access_arrange_contents,
    ),
    url(
        r"(?P<system>archivesspace|atk)/(?P<record_id>[A-Za-z0-9-_]+)/copy_to_arrange/$",
        views.access_copy_to_arrange,
    ),
    url(
        r"(?P<system>archivesspace|atk)/(?P<record_id>[A-Za-z0-9-_]+)/copy_from_arrange/$",
        views.access_arrange_start_sip,
    ),
    url(
        r"(?P<system>archivesspace|atk)/(?P<record_id>[A-Za-z0-9-_]+)/create_directory_within_arrange/$",
        views.access_create_directory,
    ),
    url(
        r"(?P<system>archivesspace|atk)/(?P<record_id>[A-Za-z0-9-_]+)/rights/$",
        views.access_sip_rights,
    ),
    url(
        r"(?P<system>archivesspace|atk)/(?P<record_id>[A-Za-z0-9-_]+)/metadata/$",
        views.access_sip_metadata,
    ),
    url(
        r"(?P<system>archivesspace|atk)/(?P<record_id>.+)/children/$",
        views.record_children,
    ),
