...
class ArchivesSpaceConfigForm(forms.Form):
    ...
    use_statement = forms.CharField(required=False, label=_('Use statement'), help_text=_('Optional, but if present should come from ArchivesSpace controlled list. Example: image-master'))
    uri_prefix = forms.CharField(label=_('URL prefix'), help_text=_('URL of DIP object server as you wish to appear in ArchivesSpace record. Example: http://example.com'))
    access_conditions = forms.CharField(required=False, label=_('Conditions governing access'), help_text=_('Populates Conditions governing access note'))
    use_conditions = forms.CharField(required=False, label=_('Conditions governing use'), help_text=_('Populates Conditions governing use note'))
    repository = forms.IntegerField(label=_('ArchivesSpace repository number'), help_text=_('Default for single repository installation is 2'), initial=2)
    inherit_notes = forms.BooleanField(required=False, label=_('Inherit digital object notes from the parent component'), initial=False)
...
class ArchivistsToolkitConfigForm(forms.Form):
    ...
    ead_actuate = forms.ChoiceField(choices=EAD_ACTUATE_CHOICES, label=_('EAD DAO actuate'), initial='none')
    ead_show = forms.ChoiceField(choices=EAD_SHOW_CHOICES, label=_('EAD DAO show'), initial='embed')
    ...
    access_conditions = forms.CharField(required=False, label=_('Conditions governing access'))
    use_conditions = forms.CharField(required=False, label=_('Conditions governing use'))
