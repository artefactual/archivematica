...
from components.administration.views_dip_upload import (
    _AS_DICTNAME,
    _ATK_DICTNAME,
    _ATOM_DICTNAME,
)
...
class TestUploadDipAsConfigView(TestCase):
    ...
    def test_post_missing_fields(self):
        response = self.client.post(self.url, {"host": "aspace.test.org", "port": 8089})
        form = response.context["form"]
        config = DashboardSetting.objects.get_dict(_AS_DICTNAME)
...
class TestProcessingConfig(TestCase):
    ...
    def test_download_404(self):
        with mock.patch(
            "components.helpers.send_file", side_effect=self.send_file_404_mock
        ):
    ...
    def test_download_ok(self):
        with mock.patch(
            "components.helpers.send_file", side_effect=self.send_file_ok_mock
        ):
            response = self.client.get(
                reverse(
                    "components.administration.views_processing.download",
    ...
    def test_edit_not_found_config(self, load_config):
        ...
        url = reverse(
            "components.administration.views_processing.edit", args=["not_found_config"]
        )
        response = self.client.get(url)

        self.assertEquals(url, "/administration/processing/" "edit/not_found_config/")
        self.assertEquals(response.status_code, 404)
    ...
    def test_edit_found_config(self, load_config):
        response = self.client.get(
            reverse(
                "components.administration.views_processing.edit", args=["found_config"]
            )
        )
