...
app_name = "ingest"
urlpatterns = [
    re_path(r"^$", views.ingest_grid, name="ingest_index"),
    re_path(r"^sips/$", views.SipsView.as_view()),
    re_path(
    ...
    re_path(r"^upload/url/check/$", views.ingest_upload_destination_url_check),
    re_path(r"^(?P<uuid>" + settings.UUID_REGEX + ")/upload/$", views.ingest_upload),
    re_path(r"^status/$", views.ingest_status),
    ...
    re_path(r"^backlog/$", views.transfer_backlog, {"ui": "legacy"}),
    re_path(r"^appraisal_list/$", views.transfer_backlog, {"ui": "appraisal"}),
]
