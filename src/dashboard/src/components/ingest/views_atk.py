...
def ingest_upload_atk_save(request, uuid):
    return pair_matcher.pairs_saved_response(
        ingest_upload_atk_save_to_db(request, uuid)
    )
...
def ingest_upload_atk_reset(request, uuid):
    ...
    return HttpResponseRedirect(
        reverse("components.ingest.views_atk.ingest_upload_atk", args=[uuid])
    )
...
def ingest_upload_atk_save_to_db(request, uuid):
    ...
    for key in keys:
        pairing = models.AtkDIPObjectResourcePairing.objects.create(
            dipuuid=pairs[key]["DIPUUID"], fileuuid=pairs[key]["objectUUID"]
        )
...
def ingest_upload_atk_match_dip_objects_to_resource_levels(request, uuid, resource_id):
    try:
        ...
        if parent_type == type(client).RESOURCE:
            ...
            parent_url = (
                "components.ingest.views_atk.ingest_upload_as_resource_component"
            )
...
def ingest_upload_atk_match_dip_objects_to_resource_component_levels(
    request, uuid, resource_component_id
):
    ...
    try:
        ...
        parent_type, parent_id = client.find_parent_id_for_component(
            resource_component_id
        )
        if parent_type == type(client).RESOURCE:
            parent_url = "components.ingest.views_atk.ingest_upload_as_resource"
