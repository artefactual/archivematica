...
def data_migration_up(apps, schema_editor):
    ...
    MicroServiceChain.objects.filter(
        description='Upload DIP to AtoM').update(
            description='Upload DIP to AtoM/Binder')
    TaskConfig.objects.filter(
        description='Choose config for AtoM DIP upload').update(
...
def data_migration_down(apps, schema_editor):
    ...
    TaskConfig = apps.get_model('main', 'TaskConfig')
    MicroServiceChain.objects.filter(
        description='Upload DIP to AtoM/Binder').update(
            description='Upload DIP to AtoM')
    TaskConfig.objects.filter(
        description='Choose config for AtoM/Binder DIP upload').update(
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('main', '0055_normalize_thumbnail_mode_selection'),
    ]
