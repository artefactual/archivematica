...
def data_migration(apps, schema_editor):
    ...
    TaskConfigSetUnitVariable = apps.get_model(
    ...
    TaskConfig.objects.filter(pk__in=(
    ...
    set_unit_variable_link_type_pk = TaskType.objects.get(
        description='linkTaskManagerSetUnitVariable').pk
    get_unit_variable_link_type_pk = TaskType.objects.get(
        description='linkTaskManagerUnitVariableLinkPull').pk

    ###########################################################################
    # Replace "assign magic link" objects
    ###########################################################################

    assign_magic_link = namedtuple('AssignMagicLink',
                                   'mscl_pk tc_pk tcmagic_pk goto_mscl_pk '
                                   'new_uuid new_var')
    assign_magic_links = [
    ...
    goto_magic_link = namedtuple('GotoMagicLink',
                                 'mscl_pk tc_pk new_uuid new_var')
    goto_magic_links = [
        goto_magic_link(
    ...
    for item in goto_magic_links:
        ...
        MicroServiceChainLink.objects \
            .filter(pk=item.mscl_pk) \
            .update(defaultexitmessage=Job.STATUS_COMPLETED_SUCCESSFULLY)
