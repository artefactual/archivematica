...
def data_migration(apps, schema_editor):
    ...
    for mscl in MicroServiceChainLink.objects.filter(currenttask=tc):
        ...
        next_mscl = MicroServiceChainLinkExitCode.objects.get(
            ...
            exitcode=0).nextmicroservicechainlink

        # Update affected chain links
        MicroServiceChainLinkExitCode.objects \
            .filter(nextmicroservicechainlink=mscl) \
            .update(nextmicroservicechainlink=next_mscl)
        MicroServiceChainLink.objects \
            .filter(defaultnextchainlink=mscl) \
            .update(defaultnextchainlink=next_mscl)

        # Update affected chains
        MicroServiceChain.objects \
            .filter(startinglink=mscl) \
            .update(startinglink=next_mscl)

        # Delete old exit codes
        MicroServiceChainLinkExitCode.objects \
            .filter(microservicechainlink=mscl) \
            .delete()
