...
def data_migration_atom_to_dict(apps, schema_editor):
    ...
    fields = ('url', 'email', 'password', 'rsync_target', 'rsync_command', 'version', 'debug', 'key')

    qs = apps.get_model('main', 'DashboardSetting').objects.filter(name__in=['{}{}'.format(old_prefix, field) for field in fields])
    old_dict = dict(qs.values_list('name', 'value'))
    new_dict = {field: old_dict.get('{}{}'.format(old_prefix, field)) for field in fields}
...
def data_migration_atom_restore_std(apps, schema_editor):
    ...
    # Update MSC
    MicroServiceChain.objects.filter(pk='0fe9842f-9519-4067-a691-8a363132ae24', description='Upload DIP to Atom').update(
...
def data_migration_as_to_dict(apps, schema_editor):
    ...
    try:
        mscrd = MicroServiceChoiceReplacementDic.objects.get(pk='f8749dd2-0923-4b57-a074-45cd92ace56f', description='ArchivesSpace Config')
...
def data_migration_atk_to_dict(apps, schema_editor):
    ...
    MicroServiceChoiceReplacementDic to joint DashboardSetting dict.
    """
    ArchivistsToolkitConfig = apps.get_model('administration', 'ArchivistsToolkitConfig')
    MicroServiceChoiceReplacementDic = apps.get_model('main', 'MicroServiceChoiceReplacementDic')
    ...
    # Attempt to read defaults from MicroServiceChoiceReplacementDic
    try:
        mscrd = MicroServiceChoiceReplacementDic.objects.get(pk='5395d1ea-a892-4029-b5a8-5264a17bbade', description='Archivists Toolkit Config')
    except MicroServiceChoiceReplacementDic.DoesNotExist:
        pass
    else:
        try:
