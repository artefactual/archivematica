...
def data_migration(apps, schema_editor):
    ...
    MicroServiceChainLinkExitCode.objects\
        .filter(microservicechainlink_id=copy_submission_docs_cl_uuid)\
        .update(nextmicroservicechainlink_id=set_bag_file_permissions_cl_uuid)
    MicroServiceChainLink.objects\
        .filter(id=copy_submission_docs_cl_uuid)\
        .update(defaultnextchainlink_id=set_bag_file_permissions_cl_uuid)
