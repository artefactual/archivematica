...
def data_migration(apps, schema_editor):
    ...
    identify_dspace_mets = "8ec0b0c1-79ad-4d22-abcd-8e95fcceabbc"

    StandardTaskConfig.objects.create(
        id="7cba173b-a631-4a2f-b49e-0627e779a7ed",
        requires_output_lock=0,
        execute="parseDataverse",
        arguments="%SIPDirectory% %SIPUUID%",
    )

    TaskConfig.objects.create(
        id="99718d08-8867-42f3-b74c-a75dc8d4c61a",
        tasktype_id="36b2e239-4a57-4aa5-8ebc-7a29139baca6",
        tasktypepkreference="7cba173b-a631-4a2f-b49e-0627e779a7ed",
        description="Parse Dataverse METS",
    )

    MicroServiceChainLink.objects.create(
        id=parse_dataverse_mscl,
        microservicegroup="Parse external files",
        defaultexitmessage="Failed",
        currenttask_id="99718d08-8867-42f3-b74c-a75dc8d4c61a",
    ...
    MicroServiceChainLinkExitCode.objects.create(
        id="c28944e8-362c-4db7-858d-f1a10ab0317c",
        microservicechainlink_id=parse_dataverse_mscl,
        exitcode=0,
    ...
    MicroServiceChainLinkExitCode.objects.filter(
        microservicechainlink_id=identify_dspace_mets
    ).update(nextmicroservicechainlink_id=parse_dataverse_mscl)
    # Update default MCL for postExtractSpecializedProcessing to point at
    # dataverse parsing
    TaskConfigUnitVariableLinkPull.objects.filter(
        id="49d853a9-646d-4e9f-b825-d1bcc3ba77f0"
    ).update(defaultmicroservicechainlink_id=parse_dataverse_mscl)
