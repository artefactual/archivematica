...
@pytest.mark.skipif(not settings.LDAP_AUTHENTICATION,
                    reason='tests will only pass if LDAP is enabled')
class TestLDAP(TestCase):
    # See settings.test for LDAP settings configuration
    top = ('o=test', {'o': ['test']})
    example = ('ou=example,o=test', {'ou': ['example']})
    alice = ('cn=alice_ldap,ou=example,o=test', {'cn': ['alice_ldap'], 'givenName': ['Alice'], 'sn': ['Example'], 'mail': ['alice@example.com'], 'userPassword': ['alicepw']})
