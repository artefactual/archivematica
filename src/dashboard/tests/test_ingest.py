...
class TestIngest(TestCase):
    ...
    def test_normalization_event_detail_view(self):
        ...
        url = reverse(
            'components.ingest.views.ingest_metadata_event_detail',
            args=[sip_uuid],
        )
    ...
    def test_add_metadata_files_view(self):
        ...
        sip_uuid = '4060ee97-9c3f-4822-afaf-ebdf838284c3'
        url = reverse(
            'components.ingest.views.ingest_metadata_add_files',
            args=[sip_uuid],
        )
        response = self.client.get(url)
        assert response.status_code == 200
