...
class RightCsvReader:
    ...
    def parse_row(self, row):
        ...
        # Use one transfer target for duplicate detection and rights statements.
        if filepath == "objects":
            filepath = self.TRANSFER_TARGET
            self.current_row["file"] = filepath
    ...
    def generate_rights_statement(self):
        ...
        if filepath == self.TRANSFER_TARGET:
            ...
            try:
                ...
                raise RightsRowException(
                    f"No file matches {filepath!r} in this transfer", self
                ) from err
