...
import type { TransferComponent, TransferFormData } from '@/shared/models'
...
type TransferBrowserComponent = {
  ...
  addSelectedPath: () => void
  handleFileSelect: (selection: { path: string, canAdd: boolean }) => void
  [key: string]: unknown
}

// Mock the useTransferAPI composable
...
vi.mock('./composables/useTransferAPI', () => {
...
describe('TransferBrowser', () => {
  ...
  describe('Source Locations', () => {
    it('auto-selects the first enabled location when multiple are available', async () => {
      ...
      const wrapper = mount(TransferBrowser, {
        global: {
          plugins: [i18nMock],
        },
      })
  ...
  describe('Button States', () => {
    ...
    it('disables the add button when the selection cannot be added', async () => {
      const wrapper = mount(TransferBrowser, {
        global: {
          plugins: [i18nMock],
        },
      ...
      const browseButton = wrapper.find('.btn-browse')
      await browseButton.trigger('click')

      transferBrowser.handleFileSelect({
        path: 'folder',
        canAdd: false,
      })
      ...
      const addButtonWrapper = wrapper.find('.add-button')
      const addButton = addButtonWrapper.element as HTMLButtonElement
      expect(addButton.disabled).toBe(true)
    })

    it('ignores add requests for selections that cannot be added', async () => {
      const wrapper = mount(TransferBrowser, {
        global: {
          plugins: [i18nMock],
        },
      ...
      transferBrowser.addSelectedPath()
