...
INSERT INTO StandardTasksConfigs (pk, execute, arguments, requiresOutputLock) VALUES (@archivesspace_upload_stc, 'upload-archivesspace_v0.0', '--host "%host%" --port "%port%" --user "%user%" --passwd "%passwd%" --dip_location "%SIPDirectory%" --dip_name "%SIPName%" --dip_uuid "%SIPUUID%" --restrictions "%restrictions%" --object_type "%object_type%" --xlink_actuate "%xlink_actuate%" --xlink_show "%xlink_show%" --use_statement "%use_statement%" --uri_prefix "%uri_prefix%" --access_conditions "%access_conditions%" --use_conditions "%use_conditions%" --inherit_notes="%inherit_notes%"', 0);
