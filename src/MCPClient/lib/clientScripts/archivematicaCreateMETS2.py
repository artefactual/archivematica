...
if __name__ == '__main__':
    ...
    # For the Dataverse (DV) proof of concept, if the transfer type is
    # 'Dataverse' then fork this work to the Dataverse POC METS2 script.
    # The DV work in that METS2 file needs to be merge into this one, or,
    # this one refactored with transfer-type specific changes then incorporated
    # in a more compartmentalised way.
    for f in File.objects.filter(sip=fileGroupIdentifier):
        if f.transfer:
            if f.transfer.type == 'Dataverse':
                logger.info("Transfer type: %s. Creating %s METS",
                             f.transfer.type, f.transfer.type)
                dataverseaip.write_dv_mets(
                  sip_type=SIP_TYPE,
                  baseDirectoryPath=baseDirectoryPath,
                  baseDirectoryPathString=baseDirectoryPathString,
                  fileGroupIdentifier=fileGroupIdentifier,
                  fileGroupType=fileGroupType,
                  xmlFile=XMLFile,
                  amdSec=includeAmdSec,
                )
                sys.exit(0)
