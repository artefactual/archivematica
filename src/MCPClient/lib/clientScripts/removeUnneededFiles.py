...
def remove_file(sip_obj_dir, sip_UUID):
    ...
    for dir_path, dir_name, filenames in os.walk(sip_obj_dir):
        for filename in filenames:
            if filename in REMOVEABLE_FILES:
    ...
    remove_files = [file_UUID for file_UUID, current_location in sip_files if (os.path.basename(current_location) in REMOVEABLE_FILES and file_UUID != 'None')]
