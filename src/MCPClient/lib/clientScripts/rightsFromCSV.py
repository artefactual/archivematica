...
class RightCsvReader:
    ...
    def parse(self):
        ...
        # Print notice if no rights CSV file found
        if not os.path.isfile(self.rights_csv_filepath):
            print "No such file:", self.rights_csv_filepath
            return 0
    ...
    def parse_row(self, row):
        ...
        if not filepath:
            self.exit_with_error('No file specified')

        # If restriction specified, insure it has an allowed value
        restriction = self.column_value('grant_restriction')
        if restriction and not restriction.lower() in ["disallow", "conditional", "allow"]:
            self.exit_with_error("The value of element restriction must be: 'Allow', 'Disallow', or 'Conditional':", restriction)
        ...
        if not act:
            self.exit_with_error('No act specified')
        ...
        if not act in self.object_basis_act_usage[filepath][basis]:
            ...
            print "Skipping duplicate basis/act combination at row", self.rows_processed + 1

        self.rows_processed += 1

    def exit_with_error(self, message, value=None):
        """Print message to standard error and exit with status 1."""
        message = '[Row {}] {}'.format(self.rows_processed + 1, message)
        if value:
    ...
    def generate_rights_statement(self):
        ...
        if basis not in dict(models.RightsStatement.RIGHTS_BASIS_CHOICES):
            self.exit_with_error('Invalid basis:', basis)
    ...
    def validate_columns(self, row):
        ...
        for column_name in row.keys():
            if not column_name in self.allowed_column_names:
                self.exit_with_error('Invalid column found:', column_name)

        for column_name in self.required_column_names:
            if not column_name in row:
                self.exit_with_error('Missing required column:', column_name)
...
if __name__ == '__main__':
    ...
    rights_csv_filepath = sys.argv[2]

    if os.path.isfile(rights_csv_filepath):
        print "Attempting to import rights data..."
        parser = RightCsvReader(transfer_uuid, rights_csv_filepath)
        rows_processed = parser.parse()
        print "Processed rows:", rows_processed
    else:
        print "No rights.csv file found at", rights_csv_filepath
