        ...
        if os.path.isfile(csv_path):
            ...
            if original is None:
                if isinstance(e, File.DoesNotExist):
                    ...
                    job.print_error("Could not find {access_file} in {filename}".format(
            ...
            if isinstance(e, File.DoesNotExist):
                job.print_error("No matching file for: ", opts.filePath.replace(opts.SIPDirectory, "%SIPDirectory%", 1))
                return 3
            elif isinstance(e, File.MultipleObjectsReturned):
                job.print_error("Too many possible files for: ", opts.filePath.replace(opts.SIPDirectory, "%SIPDirectory%", 1))
                return 2
