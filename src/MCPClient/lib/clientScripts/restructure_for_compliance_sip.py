...
def restructureForComplianceFileUUIDsAssigned(job, unit_path, unit_uuid, unit_type='sip_id', unit_path_replacement='%SIPDirectory%'):
    # Create required directories
    archivematicaFunctions.create_directories(REQUIRED_DIRECTORIES, unit_path, printing=True, printfn=job.pyprint)
    ...
    with transaction.atomic():
        for job in jobs:
            with job.JobContext():
                ...
                try:
                    restructureForComplianceFileUUIDsAssigned(job, args.target, args.sip_uuid)
