...
class PolicyChecker(object):
    ...
    def check(self):
        ...
        if not self.is_manually_normalized_access_derivative:
            try:
                ...
                self.job.pyprint('Not performing a policy check because there is no file'
        ...
        if not rules:
            self.job.pyprint('Not performing a policy check because there are no relevant'
                             ' FPR rules')
            return NOT_APPLICABLE_CODE
        rule_outputs = []
        for rule in rules:
    ...
    def _we_check_this_type_of_file(self):
        ...
        if self.file_type == 'original':
            ...
            if (not self._file_is_derivative()) or self._file_is_for_access():
                self.job.pyprint('File {uuid} is not a preservation derivative; not'
            ...
            if (self._file_is_derivative(for_access=True) and
                ...
                return True
            self.job.pyprint('File {uuid} is not an access derivative; not performing'
                             ' a policy check.'.format(uuid=self.file_uuid))
            if not self._file_is_derivative(for_access=True):
                self.job.pyprint('File {uuid} is not a derivative.'.format(
                    uuid=self.file_uuid))
            if not self._file_is_for_access():
                self.job.pyprint('File {uuid} is not for access.'.format(
                    uuid=self.file_uuid))
            return False
        else:
            return True
    ...
    def _get_rules(self):
        ...
        if self.is_manually_normalized_access_derivative:
            file_uuid = (
                self._get_manually_normalized_access_derivative_file_uuid())
        try:
    ...
    def sip_logs_dir(self):
        ...
        try:
            ...
            self.job.print_error('Warning: unable to retrieve {unit_type} model corresponding'
            ...
            if os.path.isdir(logs_dir):
                ...
                return logs_dir
            self.job.print_error('Warning: unable to find a logs/ directory in the {unit_type}'
    ...
    def sip_subm_doc_dir(self):
        ...
        try:
            ...
            if os.path.isdir(subm_doc_dir):
                ...
                return subm_doc_dir
            self.job.print_error('Warning: unable to find a metadata/submissionDocumentation/'
