...
def main(job):
        ...
        if os.path.isfile(csv_path):
            ...
            if original is None:
                if isinstance(e, File.DoesNotExist):
                    ...
                    job.print_error("Could not find {preservation_file} in {filename}".format(
            ...
            if isinstance(e, File.DoesNotExist):
                job.print_error("No matching file for: ", filePath.replace(SIPDirectory, "%SIPDirectory%", 1))
                return 3
            elif isinstance(e, File.MultipleObjectsReturned):
                job.print_error("Too many possible files for: ", filePath.replace(SIPDirectory, "%SIPDirectory%", 1))
                return 2

    # We found the original file somewhere above
    job.print_output("Matched original file %s (%s) to  preservation file %s (%s)" % (original_file.currentlocation, original_file.uuid, filePath, fileUUID))
    ...
    try:
        ...
        job.print_output('Created a manual normalization Event for file {}.'.format(
            original_file.uuid))
