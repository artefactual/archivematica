...
class TestCreateDigiprovMD(TestCase):
    ...
    def test_creates_events(self):
        ...
        assert (
            len(ret[0].findall(".//{info:lc/xmlns/premis-v2}linkingAgentIdentifier"))
            == 3
        )
        assert ret[1][0].attrib["MDTYPE"] == "PREMIS:EVENT"
        assert (
            ret[1].find(".//{info:lc/xmlns/premis-v2}eventType").text
            == "message digest calculation"
        )
        assert (
            len(ret[1].findall(".//{info:lc/xmlns/premis-v2}linkingAgentIdentifier"))
            == 3
        )
        assert ret[2][0].attrib["MDTYPE"] == "PREMIS:EVENT"
        assert (
            ret[2].find(".//{info:lc/xmlns/premis-v2}eventType").text == "virus check"
        )
        assert (
            len(ret[2].findall(".//{info:lc/xmlns/premis-v2}linkingAgentIdentifier"))
            == 3
        )
        assert ret[3][0].attrib["MDTYPE"] == "PREMIS:EVENT"
        assert (
            ret[3].find(".//{info:lc/xmlns/premis-v2}eventType").text == "name cleanup"
        )
        assert (
            len(ret[3].findall(".//{info:lc/xmlns/premis-v2}linkingAgentIdentifier"))
            == 3
        )
        assert ret[4][0].attrib["MDTYPE"] == "PREMIS:EVENT"
        assert (
            ret[4].find(".//{info:lc/xmlns/premis-v2}eventType").text
            == "format identification"
        )
        assert (
            len(ret[4].findall(".//{info:lc/xmlns/premis-v2}linkingAgentIdentifier"))
            == 3
        )
        assert ret[5][0].attrib["MDTYPE"] == "PREMIS:EVENT"
        assert ret[5].find(".//{info:lc/xmlns/premis-v2}eventType").text == "validation"
        assert (
            len(ret[5].findall(".//{info:lc/xmlns/premis-v2}linkingAgentIdentifier"))
            == 3
        )
        # Agents
        assert ret[6][0].attrib["MDTYPE"] == "PREMIS:AGENT"
        assert (
            ret[6].find(".//{info:lc/xmlns/premis-v2}agentIdentifierType").text
            == "preservation system"
        )
        assert (
            ret[6].find(".//{info:lc/xmlns/premis-v2}agentIdentifierValue").text
            == "Archivematica-1.4.0"
        )
        assert (
            ret[6].find(".//{info:lc/xmlns/premis-v2}agentName").text == "Archivematica"
        )
        assert ret[6].find(".//{info:lc/xmlns/premis-v2}agentType").text == "software"
        assert ret[7][0].attrib["MDTYPE"] == "PREMIS:AGENT"
        assert (
            ret[7].find(".//{info:lc/xmlns/premis-v2}agentIdentifierType").text
            == "repository code"
        )
        assert (
            ret[7].find(".//{info:lc/xmlns/premis-v2}agentIdentifierValue").text
            == "demo"
        )
        assert ret[7].find(".//{info:lc/xmlns/premis-v2}agentName").text == "demo"
        assert (
            ret[7].find(".//{info:lc/xmlns/premis-v2}agentType").text == "organization"
        )
        assert ret[8][0].attrib["MDTYPE"] == "PREMIS:AGENT"
        assert (
            ret[8].find(".//{info:lc/xmlns/premis-v2}agentIdentifierType").text
            == "Archivematica user pk"
        )
        assert (
            ret[8].find(".//{info:lc/xmlns/premis-v2}agentIdentifierValue").text == "1"
        )
        assert (
            ret[8].find(".//{info:lc/xmlns/premis-v2}agentName").text
            == 'username="kmindelan", first_name="Keladry", last_name="Mindelan"'
        )
        assert (
            ret[8].find(".//{info:lc/xmlns/premis-v2}agentType").text
            == "Archivematica user"
        )
...
class TestRights(TestCase):
    ...
    def test_create_rights_granted(self):
        # Setup
        elem = etree.Element(
            "{info:lc/xmlns/premis-v2}rightsStatement",
