...
class TestParseDataverse(TestCase):
    ...
    def test_mapping(self):
        ...
        for test_case in test_cases:
            assert self.mets.get_file(
                file_uuid=test_case["file_uuid"]) in mapping
            assert models.File.objects.get(
                currentlocation=test_case
                .get("file_location", "")
                .format(self.transfer_location)) in mapping.values()
        ...
        for test_case in test_cases:
            assert models.File.objects\
                .get(currentlocation=test_case
                     .get("file_location", "")
                     .format(self.transfer_location)).filegrpuse \
    ...
    def test_parse_derivative(self):
        ...
        mapping = parse_dataverse.get_db_objects(
            self.job, self.mets, self.uuid)
        agent = parse_dataverse.add_external_agents(
            self.job, self.unit_path)
    ...
    def test_validate_checksums(self):
        ...
        assert events.event_detail == \
            'program="python"; module="hashlib.md5()"'
