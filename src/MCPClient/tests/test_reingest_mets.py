...
class TestAddingNewFiles(TestCase):
    ...
    def test_new_preservation_file(self):
        ...
        amdsec = root.find('mets:amdSec[@ID="' + adm_id + '"]', namespaces=NSMAP)
        assert amdsec is not None
        print(etree.tostring(amdsec, pretty_print=True))
        # Check techMD
        ...
        assert amdsec.xpath('''.//premis:agentName[text()='username="keladry", first_name="Keladry", last_name="Mindelan"']''', namespaces=NSMAP) != []
