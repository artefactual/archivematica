...
def alreadyExistsCheck(anID):
    ...
    sql = """SELECT FileIDs FROM FileIDsByFitsFfidentMimetype where id = '%s'""" % (anID)
    rows = databaseInterface.queryAllSQL(sql)
    if len(rows) > 1:
...
def createNewID(newIDUUID, FileID, anID, fileUUID):
    ...
    description = anID
    fileIDType = '8e39a076-d359-4c60-b6f4-38f7ae6adcdf'
    ...
    sql = """INSERT INTO FileIDsByFitsFfidentMimetype (pk, FileIDs, id) VALUES ('%s', '%s', '%s');""" % (newIDUUID, FileID, anID)
    databaseInterface.runSQL(sql)
...
def printResetCommands():
    print """#DELETE CommandRelationships FROM CommandRelationships JOIN FileIDs ON CommandRelationships.FileID = FileIDs.pk WHERE FileIDs.fileIDType = '8e39a076-d359-4c60-b6f4-38f7ae6adcdf';

#DELETE FileIDsByFitsFfidentMimetype FROM FileIDsByFitsFfidentMimetype;

#DELETE FileIDs FROM FileIDs  WHERE FileIDs.fileIDType = '8e39a076-d359-4c60-b6f4-38f7ae6adcdf';"""
