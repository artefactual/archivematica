...
def alreadyExistsCheck(anID):
    ...
    sql = """SELECT FileIDs FROM FileIDsByFitsFitsFormat where id = '%s'""" % (anID)
    rows = databaseInterface.queryAllSQL(sql)
    if len(rows) > 1:
...
def createNewID(newIDUUID, FileID, anID, fileUUID):
    ...
    sql = """INSERT INTO FileIDsByFitsFitsFormat (pk, FileIDs, id) VALUES ('%s', '%s', '%s');""" % (newIDUUID, FileID, anID)
    databaseInterface.runSQL(sql)
...
#DELETE FileIDsByFitsFitsFormat FROM FileIDsByFitsFitsFormat;

#DELETE FileIDs FROM FileIDs  WHERE FileIDs.fileIDType = 'cff7437f-20c6-440a-b801-37c647da2cf1';"""
