...
def alreadyExistsCheck(anID):
    ...
    sql = """SELECT FileIDs FROM FileIDsByFitsFitsMimetype where id = '%s'""" % (anID)
    rows = databaseInterface.queryAllSQL(sql)
    if len(rows) > 1:
...
def createNewID(newIDUUID, FileID, anID, fileUUID):
    ...
    description = anID
    fileIDType = 'c26227f7-fca8-4d98-9d8e-cfab86a2dd0a'
    ...
    sql = """INSERT INTO FileIDsByFitsFitsMimetype (pk, FileIDs, id) VALUES ('%s', '%s', '%s');""" % (newIDUUID, FileID, anID)
    databaseInterface.runSQL(sql)
...
def printResetCommands():
    print """#DELETE CommandRelationships FROM CommandRelationships JOIN FileIDs ON CommandRelationships.FileID = FileIDs.pk WHERE FileIDs.fileIDType = 'c26227f7-fca8-4d98-9d8e-cfab86a2dd0a';

#DELETE FileIDsByFitsFitsMimetype FROM FileIDsByFitsFitsMimetype;

#DELETE FileIDs FROM FileIDs  WHERE FileIDs.fileIDType = 'c26227f7-fca8-4d98-9d8e-cfab86a2dd0a';"""
